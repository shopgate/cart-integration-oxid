...
use Shopgate\Oxid\Model\DeliveryList;
use Shopgate\Oxid\Shopgate\Config;
use Shopgate\Oxid\Shopgate\Plugin;
...
class Module
{
    ...
    public static function getInstance()
    {
        ...
        if (!(self::$instance instanceof Module)) {
            self::$instance = oxNew('ShopgateModule');
            self::$instance->init();
        }
     ...
     *
     * @param Module $oNewInstance
     *
     * @return Module
     */
    public static function replaceInstance(Module $oNewInstance)
    {
    ...
    {
        return ShopgateModule . phpShopgateModule::getOxConfig()->getConfigParam('sShopDir') . DIRECTORY_SEPARATOR . self::FRAMEWORK_DIR . DIRECTORY_SEPARATOR;
    }
    ...
    public function getConfigForAdminGui()
    {
        ...
        $this->_aConfig['language']['options'] = Module::getOxLang()->getLanguageNames();

        $folders = Module::getOxConfig()->getConfigParam('aOrderfolder');
    ...
    public static function cleanUrl($url)
    {
        if (class_exists('oxUtilsUrl') && method_exists('oxUtilsUrl', 'cleanUrl')) {
            return Module::getOxUtilsUrl()->cleanUrl($url);
        }
        $oStr = getStr();
    ...
    {
        $result = DatabaseProvider::getDb(true)->getAll($sql, $params);
    ...
    {
        $result = DatabaseProvider::getDb()->GetOne($sql, $params);
    ...
    {
        // todo check which return value callers expect, was hinted to be object, is now (and probably has always been) integer
        return DatabaseProvider::getDb()->Execute($sql);
    }
    ...
    {
        if (version_compare(Module::getOxConfig()->getVersion(), '4.3.0', '<')) {
            return '';
        }
        /** @var oxState $oxState */
    ...
    {
        /** @noinspection PhpUndefinedClassInspection PhpDeprecationInspection */
