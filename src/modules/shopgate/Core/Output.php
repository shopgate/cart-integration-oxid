...
use marm_shopgate_oxoutput_parent;
use oxView;
use oxWidget;
use Search;
use ShopgateBuilder;
...
class Output extends Output_parent
{
    ...
    public function process($sValue, $sClassName)
    {
        ...
        if (!$this->isAjax() && $sClassName != 'oxemail') {
            if (!isAdmin()) {
                ...
                $version = Module::getOxConfig()->getVersion();
                if (
    ...
    protected function getGrantRedirect(oxView $oActiveView)
    {
        ...
        $selectedLang = Module::getOxLang()->getLanguageAbbr();
    ...
    protected function makeMobileRedirect()
    {
        ...
        $oActiveView = Module::getOxConfig()->getActiveView();
        ...
        if ($viewClass == 'content') {
            ...
            $languageId = Module::getOxLang()->getBaseLanguage();
