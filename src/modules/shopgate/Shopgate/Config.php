...
class Config extends ShopgateConfig
{
    ...
    public function startup()
    {
        ...
        $iUtfMode = Module::getOxConfig()->getConfigParam('iUtfMode');
    ...
    {
        $oxConfig = Module::getOxConfig();

        $result = array();
        foreach (Module::getInstance()->_getConfig() as $sConfigKey => $aOptions) {
            $sValue = $oxConfig->getConfigParam(Module::getInstance()->getOxidConfigKey($sConfigKey));
            if ($sValue !== null) {
                $result[$sConfigKey] = $sValue;
            }
        }

        $shopDir            = Module::getOxConfig()->getConfigParam('sShopDir');
        ...
        if (version_compare(Module::getOxConfig()->getVersion(), '4.2.0', '<')) {
        ...
        $mobileHeaderParent  = $oxConfig->getConfigParam(
            Module::getInstance()->getOxidConfigKey('mobile_header_parent')
        );
        $mobileHeaderPrepend = $oxConfig->getConfigParam(
            Module::getInstance()->getOxidConfigKey('mobile_header_prepend')
        );
    ...
    {
        $oxConfig     = Module::getOxConfig();
        $configFields = Module::getInstance()->_getConfig();
        ...
        foreach ($fieldList as $field) {
            ...
            $oxidName = Module::getInstance()->getOxidConfigKey($field);
    ...
    public function isVariantParentBuyable()
    {
        if ($this->variant_parent_buyable == 'oxid') {
            return Module::getOxConfig()->getConfigParam('blVariantParentBuyable');
        }
