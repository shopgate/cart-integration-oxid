...
class ShopgateUserHelper extends ShopgateObject
{
    ...
    public function getOxidUserByEmail(ShopgateCartBase $shopgateOrder)
    {
        ...
        if (!$oxUserId || !$oxUser->load($oxUserId)) {
            ...
            if (_SHOPGATE_ACTION == Plugin::ACTION_CHECK_CART) {
                ...
                $invoiceAddress->setLastName(User::CHECK_CART_USERNAME);
    ...
    protected function modifyUserForOrder(oxUser $oxUser, ShopgateCartBase $shopgateOrder)
    {
        ...
        foreach ($oxAddresses as $oxAddress) {
            if ($this->compareAddresses($oxDeliveryAddress, $oxAddress, $addressCompareFields)) {
                ...
                Module::setSessionVar('deladrid', $oxAddress->getId());
        ...
        if (!$foundAddress) {
            ...
            /** @noinspection PhpUndefinedMethodInspection */
            Module::setSessionVar('deladrid', $oxDeliveryAddress->getId());
            if (method_exists('oxUser', 'setSelectedAddressId')) {
    ...
    public function getIdByUserName($userName)
    {
        ...
        if (!Module::getOxConfig()->getConfigParam('blMallUsers')) {
            $sQ .= " AND `oxshopid` = " . $oDb->quote(Module::getOxConfig()->getShopId());
        }

        return Module::dbGetOne($sQ);
    }
    ...
    {
        $homeCountry = Module::getOxConfig()->getConfigParam('aHomeCountry');
        if (Module::getOxConfig()->getConfigParam('blCalculateDelCostIfNotLoggedIn') && is_array($homeCountry)) {
    ...
    private function checkUserForMissingShopId($oxUserId)
    {
        ...
        $shopId = Module::dbGetOne("SELECT oxshopid FROM oxuser WHERE oxid = '$oxUserId'");
        if (!empty($shopId)) {
            // User already has a shopId
            return;
        }
        $shopIds = Module::dbGetAll("SELECT oxid FROM oxshops;");
        if (count($shopIds) > 1) {
        ...
        Module::dbExecute("UPDATE oxuser SET oxshopid = '$shopId' WHERE oxid = '$oxUserId'");
    }
    ...
    {
        return Module::dbGetOne("SELECT OXID FROM oxcountry WHERE OXISOALPHA2 = ?", array($sShopgateCountryId));
    }
    ...
    {
        return Module::dbGetOne("SELECT OXID FROM oxstates WHERE OXISOALPHA2 = ?", array($sShopgateStateId));
    }
