...
class ShopgateItemExportHelper
{
    ...
    /**
     * @param Config $config
     * @param Module      $marm_shopgate $marm_shopgate
     */
    public function __construct(Config $config, Module $marm_shopgate)
    {
        $this->config       = $config;
     ...
     * @return string
     */
    private function getItemNumberSuffix(oxArticle $oxArticle, $isChild, Config $config)
    {
    ...
    public function getName(oxArticle $oxArticle)
    {
        ...
        if ($type == Config::ARTICLE_NAME_EXPORT_TYPE_BOTH && !empty($oxArticle->oxarticles__oxshortdesc->value)) {
            $name = $oxArticle->oxarticles__oxtitle->value . ' ' . $oxArticle->oxarticles__oxshortdesc->value;
        } elseif ($type == Config::ARTICLE_NAME_EXPORT_TYPE_SHORTDESC && !empty($oxArticle->oxarticles__oxshortdesc->value)) {
    ...
    private function getTagsByArticleTagList($oxArticle)
    {
        ...
        $oArticleTagList->loadInLang(Module::getOxLang()->getEditLanguage(), $oxArticle->getId());
