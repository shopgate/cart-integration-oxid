...
namespace Shopgate\Oxid\Shopgate;

use Shopgate\Oxid\Model\Basket;
use Shopgate\Oxid\Model\ShopgateOrder;
use Shopgate\Oxid\Model\User;
use ShopgatePlugin;
...
require_once dirname(__FILE__) . '/export/item.php';
require_once dirname(__FILE__) . '/export/category.php';
require_once dirname(__FILE__) . '/export/review.php';
...
class Plugin extends ShopgatePlugin
{
    ...
    private $moduleEntries44 = array(
        ...
        'Shopgate\Oxid\Controller\Admin\Article' => 'shopgate/marm_shopgate_oxadminview',
        'Shopgate\Oxid\Controller\Admin\Order' => 'shopgate/marm_shopgate_oxadminview',
        'Shopgate\Oxid\Controller\Admin\Shipping' => 'shopgate/marm_shopgate_oxadminview',
        'Shopgate\Oxid\Controller\Admin\Payment' => 'shopgate/marm_shopgate_oxadminview',
    );
    ...
    public function startup()
    {
        ...
        $this->itemExportHelper  = new ShopgateItemExportHelper($this->config, Module::getInstance());
        ...
        Module::getOxLang()->setBaseLanguage($this->config->getLanguage());
    ...
    {
        $shopId = Module::getOxConfig()->getShopId();
        if (empty($shopId)) {
            $shopId = 'oxbaseshop';
        }
        /** @var oxShop $oOxShop */
    ...
    private function checkModuleEntries()
    {
        ...
        $oxidModules = Module::getOxConfig()->getConfigParam('aModules');
    ...
    {
        if (version_compare(Module::getOxConfig()->getVersion(), '4.6.0', ">=")) {
            return array();
        }
        if (version_compare(Module::getOxConfig()->getVersion(), '4.5.0', ">=")) {
    ...
    private function checkDbTables()
    {
        ...
        foreach ($this->dbTables as $table) {
            $exists         = Module::dbGetOne("SHOW TABLES LIKE '$table'");
            $result[$table] = $exists
                ? 'OK'
                : 'MISSING';
        }
    ...
    private function checkDbColumn($table, $column, $type)
    {
        try {
            return
                (bool)Module::dbGetOne("SHOW COLUMNS FROM $table WHERE field = '$column'")
                || (bool)Module::dbExecute(
    ...
    private function checkDbEntries()
    {
        ...
        foreach ($this->dbEntries as $table => $oxIDs) {
            foreach ($oxIDs as $oxID) {
                $exists                 = Module::dbGetOne("SELECT oxid FROM $table WHERE oxid = '$oxID'");
    ...
    {
        return array(
            'category_count'    => Module::dbGetOne('SELECT count(OXID) FROM oxcategories'),
            'item_count'        => Module::dbGetOne('SELECT count(OXID) FROM oxarticles'),
            'review_count'      => Module::dbGetOne('SELECT count(OXID) FROM oxreviews'),
    ...
    private function getInstalledPlugins()
    {
        ...
        if (class_exists('oxModuleList')) {
            ...
            $modules = $oxModuleList->getModulesFromDir(Module::getOxConfig()->getModulesDir());
        ...
        $moduleVersions = Module::getOxConfig()->getConfigParam('aModuleVersions');
    ...
    {
        /** @var ShopgateOrder $oBaseOxOrderShopgate */
    ...
    private function getReviewIds($limit = null, $offset = null, array $uids = array())
    {
        ...
        $sql      .= Module::getOxConfig()->getConfigParam('blGBModerate')
            ? " AND OXACTIVE = 1"
            : '';
        $sql      .= !empty($uids)
            ? " AND oxid in ('" . implode("','", $uids) . "')"
    ...
    private function checkIfArticlesExist(ShopgateOrder $order)
    {
        ...
        foreach ($order->getItems() as $item) {
            ...
            if (!empty($info['article_oxid'])) {
                ...
                $oxid      = Module::dbGetOne($sql, array($info['article_oxid']));
                if (empty($oxid)) {
    ...
    public function updateOrder(ShopgateOrder $order)
    {
        ...
        /** @var ShopgateOrder $oOxidOrderShopgate */
        ...
        $sOxidOrderId = Module::dbGetOne($qry);
    ...
    protected function getActiveCurrency()
    {
        ...
        $oCur             = Module::getOxConfig()->getActShopCurrencyObject();
    ...
    public function checkCart(ShopgateCart $cart)
    {
        ...
        if (!Module::getOxConfig()->getConfigParam('iUtfMode')) {
    ...
    private function getPaymentMethods(ShopgateCart $cart)
    {
        ...
        Module::setSessionVar('sShipSet', $shippingServiceId);
     ...
     */
    private function getShippingMethods(Basket $oxBasket)
    {
        /** @noinspection PhpParamsInspection */
        /** @var oxDeliverySet[] $oxDeliverySets */
        $oxDeliverySets = Module::getOxDeliverySetList()->getDeliverySetData(
            null,
            $oxBasket->getUser(),
            $oxBasket
        );
        ...
        if (!empty($oxDeliverySets)) {
            foreach ($oxDeliverySets as $oxDeliverySet) {
                Module::getOxDeliveryList()->sg_reset();
     ...
     *@throws object
     */
    protected function checkCoupons(Basket $oxBasket, ShopgateCart $cart)
    {
        $voucherStack = array();
    ...
    protected function redeemVouchers(ShopgateCartBase $cart)
    {
        foreach ($cart->getExternalCoupons() as $coupon) {
            ...
            if ($info) {
                ...
                $oxVoucher->oxvouchers__oxdateused->setValue(date('Y-m-d', Module::getOxUtilsDate()->getTime()));
                $oxVoucher->save();
            }
        }
    }
    ...
    public function registerCustomer($user, $pass, ShopgateCustomer $customer)
    {
        ...
        $oxUser->oxuser__oxshopid    = new oxField(Module::getOxConfig()->getShopId(), oxField::T_RAW);
